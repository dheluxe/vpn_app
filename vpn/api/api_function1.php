...
include_once(ROOT_PATH.'/includes/phpseclib/Net/Telnet.php');
...
function get_acl_info($id){
    ...
    if($res->num_rows > 0 || $res_shared_acl->num_rows > 0){
        ...
        return array('status'=>1, 'data'=>$acl_info, 'message_type'=>'reply', "type"=>"get_acl_info","value"=>$cur_tunnel_id);
    }
    else{
        return array('status' =>0 , 'data'=>'No ACL found for this Tunnel',"value"=>$cur_tunnel_id);
    }
...
function readLabel($key) {
    ...
    else if($key[1] == 'deny_allow_all'){
        $result = array(
            'full' => 'Deny all/Allow all',
            'short' => 'E'
        );
    }
    else if($key[0] == 'destination')
    {
        if($key[1] == 'real_ip'){
            $result = array(
                'full' => 'Real Ip',
...
function create_new_acl($data){
    ...
    if($res==1){
        return array("status" => 1, 'data' => 'Your request under process, please wait...', 'message_type'=>'reply', 'type'=>'create_new_acl', 'value'=>$data['id']);
    }
}
...
function save_acl_name_description($data){
    ...
    if($db->query($sql)){
        return array("status" => 1, 'data' => $data, 'message_type'=>'reply', 'type'=>'save_acl_name_description', 'value'=>$data['acl_id']);
    }
}
function change_searchable($data){ //for websocket
    global $db;
    $result=array("type"=>"change_searchable", "message_type"=>"reply", "data"=>array("status"=>"0","data"=>$data));
    $sql="";
...
function create_acl_clone($data){
    ...
    if($res==1){
        return array("status" => 1, 'data' => 'Your request under process, please wait...', 'message_type'=>'reply', 'type'=>'create_acl_clone', 'value'=>$data['tid']);
    }
}
function delete_acl($data){
    ...
    if($res==1){
        return array("status" => 1, 'data' => 'Your request under process, please wait...', 'message_type'=>'reply', 'type'=>'delete_acl', 'value'=>$data['id']);
    }
...
function set_default_acl($data){
    global $db;
    if($db->query("UPDATE `tunnels_data` SET `default_acl`=".$data['id']." WHERE `tunnel_id`=".$data['tid']))
        return array("status" => 1, 'data' => 'This acl is set as default acl.', 'message_type'=>'reply', 'type'=>'set_default_acl', 'id'=>$data['id'],'tid'=>$data['tid']);
}
function point($data){
    global $db;
...
function install_acl($data, $u_id){
    global $db;
    $select_sql="SELECT * FROM `source` WHERE `acl_id`=".$data['acl_id'];
    $query=$db->query($select_sql);
    if($query->num_rows>0){
        $source_tunnels=$query->fetch_assoc()['source-specific_tunnel'];
        if($db->query("INSERT INTO `user_acl_relation` (`acl_id`,`user_id`,`tunnel_id`) VALUES(".$data['acl_id'].",".$u_id.",".$data['tunnel_id'].")")){
            if($source_tunnels==""){
                $source_tunnels=$data['tunnel_id'];
            }else{
                $source_tunnels=$source_tunnels.":".$data['tunnel_id'];
            }
            $db->query("UPDATE `source` SET `source-specific_tunnel`='".$source_tunnels."' WHERE `acl_id`=".$data['acl_id']);
        }
        return 1;
    }else{
        return 0;
                    ...
                    <div class="content" style="padding-top: 0px;">
                        ...
                        <div data-uid="<?php echo $_SESSION['user_id'] ?>" class="just list Parks">
                            ...
                            <div class="list_header">
                                ...
                                <div class="meta width-60" data-toggle="tooltip" data-placement="bottom" title="Points" style="margin-left: 4px;"><i class="fa fa-fw fa-dollar"></i></div>
                                <div class="meta width-140" style="width: 137px!important;" data-toggle="tooltip" data-placement="bottom" title="Real IP">Real IP</div>
                                <div class="meta width-60" data-toggle="tooltip" data-placement="bottom" title="Gateway mode">Gateway</div>
                                <div class="meta"></div>
                            </div>
                            <form id="tunnels_form_field" style="display:none;">
                                <input type="button" class="btn btn-sm btn-primary btn_add_tunnel" data-cloud="<?php echo $cloud_id; ?>" value="Submit">
...
function remove_sharing($data){ //for websocket
    global $db;
    $result= array('status'=>0, 'data'=>'Your request under process, please wait...', 'message_type'=>'reply', 'type'=>'remove_sharing', 'data'=>$data);
...
function get_badge_cnt($token){
    global $db;
    $customer_data=get_customer_data_from_token($token);
    $friends_sql="SELECT id FROM `friends_data` WHERE (friend_id=".$customer_data['customer_id']." OR customer_id=".$customer_data['customer_id'].") AND status='accepted'";
    $request_friends_sql="SELECT id FROM `friends_data` WHERE (friend_id=".$customer_data['customer_id']." OR customer_id=".$customer_data['customer_id'].") AND status='request'";
    $rejected_friends_sql="SELECT id FROM `friends_data` WHERE (friend_id=".$customer_data['customer_id']." OR customer_id=".$customer_data['customer_id'].") AND status='rejected'";
    ...
    $result= array('friends_cnt'=>$friends_cnt,'request_friends_cnt'=>$request_friends_cnt,'rejected_friends_cnt'=>$rejected_friends_cnt);
    return array('status'=>1, 'data'=>$result, 'message_type'=>'reply', 'type'=>'get_badge_cnt');
...
function set_friend($data){
    ...
    if($status=="request"){
        ...
        if($res->num_rows==0){
            ...
            $res=$db->query($sql);
            return array('status'=>1, 'message'=>'Your request has been sent successfully.', 'message_type'=>'reply', 'type'=>'set_friend', 'data'=>$data);
        }
...
function get_request_friend_list($data){
    global $db;
    $result= array('status'=>1,'message_type'=>'reply', 'type'=>'get_request_friend_list', 'data'=>$data);
    $token=$data['token'];
    $customer_info=get_customer_data_from_token($token);
    $sql_request_friends=$db->query("SELECT * FROM `friends_data`
            LEFT JOIN `customers_data` ON `friends_data`.`customer_id`=`customers_data`.`customer_id`
            WHERE `friends_data`.`friend_id`='".$customer_info['customer_id']."' AND `friends_data`.`status`='request'");
    $all_friends=array();
    while($row=$sql_request_friends->fetch_assoc()){
        $get_customer_acl_destination=get_customer_acl_destination($row['customer_id']);
        $row['shared_acl_cnt']=count($get_customer_acl_destination);
        $all_friends['owner_list'][]=$row;
    }

    $sql_request_friends=$db->query("SELECT * FROM `friends_data`
            LEFT JOIN `customers_data` ON `friends_data`.`friend_id`=`customers_data`.`customer_id`
            WHERE `friends_data`.`customer_id`='".$customer_info['customer_id']."' AND `friends_data`.`status`='request'");
    while($row=$sql_request_friends->fetch_assoc()){
        $get_customer_acl_destination=get_customer_acl_destination($row['customer_id']);
        $row['shared_acl_cnt']=count($get_customer_acl_destination);
        $all_friends['other_list'][]=$row;
    }
    $result['data']['friend_list']=$all_friends;
    return $result;
}
...
function get_rejected_friend_list($data){
    global $db;
    $result= array('status'=>1,'message_type'=>'reply', 'type'=>'get_rejected_friend_list', 'data'=>$data);
    $token=$data['token'];
    $customer_info=get_customer_data_from_token($token);
    $sql_rejected_friends=$db->query("SELECT * FROM `friends_data`
                LEFT JOIN `customers_data` ON `friends_data`.`customer_id`=`customers_data`.`customer_id`
                WHERE `friends_data`.`friend_id`='".$customer_info['customer_id']."' AND `friends_data`.`status`='rejected'");
    $all_friends=array();
    ...
    $sql_rejected_friends=$db->query("SELECT * FROM `friends_data`
                LEFT JOIN `customers_data` ON `friends_data`.`friend_id`=`customers_data`.`customer_id`
                WHERE `friends_data`.`customer_id`='".$customer_info['customer_id']."' AND `friends_data`.`status`='rejected'");
...
function get_friend_list($data) //for websocket
{
    global $db;
    $result= array('status'=>1,'message_type'=>'reply', 'type'=>'get_friend_list', 'data'=>$data);
    $token=$data['token'];
    $friend_ids=array();
    $customer_info=get_customer_data_from_token($token);
    $sql_friends=$db->query("SELECT * FROM `friends_data` WHERE (`customer_id`='".$customer_info['customer_id']."' OR `friend_id`='".$customer_info['customer_id']."') AND `status`='accepted'");
    if($sql_friends->num_rows>0){
        while($row=$sql_friends->fetch_assoc()){
            if($row['friend_id']!=$customer_info['customer_id']){
                $friend_ids[]=$row['friend_id'];
            }else{
                $friend_ids[]=$row['customer_id'];
            }
...
function destroy_account($token){
    ...
    if($customer_query->num_rows>0){
        ...
        return array("status" => 1, 'data' => 'This account is destroyed.', "type"=>"destroy_account",);
    }
}
...
function save_diagram_data($data){
    ...
    $links_array=array();
    $select_links_sql="SELECT `from_node`,`to_node` FROM `diagram_links`";
    $select_links_query=$db->query($select_links_sql);
    if($select_links_query->num_rows>0){
        while($row=$select_links_query->fetch_assoc()){
            $links_array[]=$row;
        }
    }
    ...
    foreach($diagram_data_arr['linkDataArray'] as $link){
        ...
        $db->query($insert_sql);
        $new_links_array[]=array('from_node'=>$from_node,'to_node'=>$to_node);
        if(!in_array(array('from_node'=>$from_node,'to_node'=>$to_node),$links_array)){
            $result.=install_vpn_connection($from_node,$to_node);/////////////////////////install vpn connection
            $result.=" ";
        }
    }
    foreach($links_array as $link){
        if(!in_array($link,$new_links_array)){
            $result.=remove_vpn_connection($link['from_node'],$link['to_node']);/////////////////////////remove vpn connection
            $result.=" ";
        }
    }
...
function install_vpn_connection($from_node,$to_node){

    $from_remote_server=get_remote_server_detail($from_node);
    $from_remote_server_vpn_ip=create_vpn_ip_from_server_id($from_node);
    $from_remote_server_ip=$from_remote_server['remote_ip'];

    $to_remote_server=get_remote_server_detail($to_node);
    $to_remote_server_vpn_ip=create_vpn_ip_from_server_id($to_node);
    $to_remote_server_ip=$to_remote_server['remote_ip'];

    $command_for_from_server="openvpn  --remote ".$to_remote_server_ip." --dev tun1 --proto udp --ifconfig  ".$from_remote_server_vpn_ip." ".$to_remote_server_vpn_ip." --tun-mtu 48000 --fragment 0 --mssfix 0  --cipher aes-256-cbc --engine aesni";
    $command_for_to_server="openvpn  --remote ".$from_remote_server_ip." --dev tun1 --proto udp --ifconfig  ".$to_remote_server_vpn_ip." ".$from_remote_server_vpn_ip." --tun-mtu 48000 --fragment 0 --mssfix 0  --cipher aes-256-cbc --engine aesni";

    /////////////////////////////////////////////////////////////
    $ip=$from_remote_server['remote_ip'];
    $ssh_username=$from_remote_server['ssh_username'];
    $ssh_password=$from_remote_server['ssh_password'];
    $ssh = new Net_SSH2($ip);
    if (!$ssh->login($ssh_username, $ssh_password)) {
        return "failed";
    }
    $ssh->exec("apt-get install openvpn -y");
    $ssh->exec($command_for_from_server . " > /dev/null &");
    $ssh->disconnect();
/////////////////////////////////////////////////////////////////
    $ip=$to_remote_server['remote_ip'];
    $ssh_username=$to_remote_server['ssh_username'];
    $ssh_password=$to_remote_server['ssh_password'];
    $ssh = new Net_SSH2($ip);
    if (!$ssh->login($ssh_username, $ssh_password)) {
        return "failed";
    }
    $ssh->exec("apt-get install openvpn -y");
    $ssh->exec($command_for_to_server . " > /dev/null &");
    //$ssh->exec($command_for_to_server);
    $ssh->disconnect();
    sleep(2);
    return $command_for_from_server." ".$command_for_to_server;
}
function get_remote_server_detail($id){
    global $db;
    $remote_server_data_sql="SELECT * FROM `remote_server_list` WHERE `id`=".$id;
    $remote_server_data_query=$db->query($remote_server_data_sql);
    $result=array();
    if($remote_server_data_query->num_rows>0){
        $result=$remote_server_data_query->fetch_assoc();
    }
    return $result;
}
function create_vpn_ip_from_server_id($id){
    $i=intval($id);
    $a1="192";
    $a2="168";
    $a3=($i-$i%255)/255;
...
function remove_vpn_connection($from_node,$to_node){
    $from_remote_server=get_remote_server_detail($from_node);
    $from_remote_server_vpn_ip=create_vpn_ip_from_server_id($from_node);
    $from_remote_server_ip=$from_remote_server['remote_ip'];
    ...
    $command_for_from_server="kill $(ps -ax | grep ".$to_remote_server_vpn_ip." | awk '{print $1}')";
    $command_for_to_server="kill $(ps -ax | grep ".$from_remote_server_vpn_ip." | awk '{print $1}')";
    ...
    $ssh->exec($command_for_from_server);
    $ssh->disconnect();
...
function run_ospf($data){
    global $db,$Server;
    $result="";
    $vpn_ip=create_vpn_ip_from_server_id($data['id']);
    $remote_data_query=$db->query("SELECT * FROM `remote_server_list` WHERE `id`=".$data['id']);
    if($remote_data_query->num_rows>0){
        $remote_data=$remote_data_query->fetch_assoc();
        $ip=$remote_data['remote_ip'];
        ...
        $tlt=new Telnet($ip,'2604');
        if($tlt->login($tel_username,$tel_password)){
            return "telnet login failed";
        }
        $result.=$tlt->exec("enable");
        $result.=$tlt->exec("configure terminal");
        $result.=$tlt->exec("router ospf");
        $result.=$tlt->exec("router-id ".$data['id']);
        $result.=$tlt->exec("net ".$vpn_ip."/32 area 0");
        $result.=$tlt->disconnect();
        return $vpn_ip.":".$result;
    }
    return 'telnet login failed';
}
function stop_ospf($data){
    global $db;
    $result="";
    $vpn_ip=create_vpn_ip_from_server_id($data['id']);
    $remote_data_query=$db->query("SELECT * FROM `remote_server_list` WHERE `id`=".$data['id']);
    if($remote_data_query->num_rows>0){
        ...
        $result.=$tlt->exec("router ospf");
        $result.=$tlt->exec("router-id ".$data['id']);
        $result.=$tlt->exec("no net ".$vpn_ip."/32 area 0");
        $result.=$tlt->disconnect();
        return $vpn_ip.":".$result;
    }
    return 'telnet login failed';
